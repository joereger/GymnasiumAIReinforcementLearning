...
def plot_training_data(episode_stats, filename="pong_double_dqn_progress.png"):
    """
    Create a visualization of Double DQN training progress with multiple metrics.
    ...
    fig.suptitle('Pong Double DQN Training Progress (Experiment 2)', fontsize=16)
    ...
    # Add a horizontal line at reward 0 to highlight when we start winning
    if min(rewards) < 0 and max(rewards) > 0:
        ax1.axhline(y=0, color='gray', linestyle='-', alpha=0.3)
    
    # Add experiment details in a text box
    textstr = '\n'.join([
        'Double DQN Experiment 2:',
        'LR: 1e-5',
        'Target Update: 1,000 steps', 
        'Replay Buffer: 500K',
        'Epsilon Decay: 2M frames'
    ])
